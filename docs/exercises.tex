...
\subsubsection{Copying files from a remote machine (like LXplus)}
\label{sec:scp}
...
Use \texttt{scp} from a terminal \emph{on your local machine} (this works on Linux, macOS, and Windows with PowerShell or WSL):
...
Then open the \texttt{.svg} files in your browser locally.
...
\texttt{perf} & \texttt{/usr/bin/local/perf} \\
\bottomrule
\end{tabular}
\end{center}
...
\label{sec:installation_flamegraph}
...
Before choosing a CUDACPP backend to run, check the compiler flags by reading \texttt{/proc/cpuinfo}, or by running \texttt{lscpu}, you should find that at least \texttt{sse4\_2} and \texttt{avx}/\texttt{avx2}.
...
\begin{tabular}{llll}
\toprule
\textbf{CPU flag(s)} & \textbf{MadGraph Backend} & \textbf{Width} & \textbf{Doubles/instruction} \\
\midrule
\texttt{sse4\_2}  & \texttt{cppsse4} & 128-bit & 2 \\
\texttt{avx2}     & \texttt{cppavx2} & 256-bit & 4 \\
\texttt{avx512f} or \texttt{avx512bw} or \texttt{avx512vl} & \texttt{cpp512y} or \texttt{cpp512z} & 512-bit & 8 \\
\bottomrule
\end{tabular}
\end{center}
...
Check which SIMD instruction sets your machine supports and decide which backend you will use in the following exercises (whenever you find the placeholder \texttt{<backend>}, the value of the parameter \texttt{cudacpp\_backend}).
...
\begin{itemize}[nosep]
    ...
    \item \texttt{madevent\_gpu}: enables GPU backends (\texttt{cuda}, \texttt{hip})
\end{itemize}
\end{tipbox}
...
Restart MadGraph if you closed it, and run the following commands (replace \texttt{<backend>} with the appropriate backend for your machine, see~\S~\ref{sec:check_simd}):
...
The \texttt{done} command in the middle serves for the purpose of skipping one of the prompts, while the one at the end is used to start the event generation.
The \texttt{set} command can be used only in the second prompt, when the program is asking to modify \texttt{param\_card.dat} or \texttt{run\_card.dat}.
...
When using \texttt{launch}, MadGraph orchestrates the run of the executable \texttt{madevent} on the different parametrisations determined by the \texttt{G*/} folders for a certain subprocess.
...
Remember to substitute \texttt{<backend>} with whatever you chose in \S~\ref{sec:check_simd}.
...
Let's create a new input file \texttt{input\_app.txt}, where we increase the number of events to at least \textbf{100\,000} for CPU runs (the first integer in the first line), and \textbf{1\,000\,000} for GPU runs (in case you are trying these exercises on the GPU).
...
\begin{warnbox}[What if it can't find \texttt{stackcollapse-perf.pl}]
This may happen because of two reasons:
\begin{itemize}
    \item you did not downloaded the \texttt{Flamegraph} repository as explained in \S~\ref{sec:installation_flamegraph};
    \item you happened to have closed the current terminal, so now the environment variable \texttt{\$FLAMEGRAPH\_DIR} is empty.
\end{itemize}
Have a look at \S~\ref{sec:installation_flamegraph}, and perform the installation or define the environment variable for the current session, if needed.
For the latter, in case you downloaded the \texttt{Flamegraph} repository in your home directory, you can run the following:
\begin{lstlisting}[style=bashstyle]
...
Open \texttt{flamegraph\_fortran.svg} in a web browser (if you are working on a remote machine, remember to copy the SVG files to your laptop in order to view them in a browser, see \S~\ref{sec:scp}).
...
Now profile the vectorised C++ version (replace \texttt{madevent\_cpp} with the appropriate binary name, in case you are trying these exercises on GPU):
...
In the following, you need to replace the placeholders \texttt{<FP\_XXX>} with the appropriate counter names for your CPU.
...
Collect the counters for both versions and fill in the following table (for the counters you have, you may not have some of them available on your system):
\bigskip
\begin{center}
